...
ele_dry_reg <- readRDS(here("outputs", "ele_dry_reg.rds"))
ele_wet_reg <- readRDS(here("outputs", "ele_wet_reg.rds"))
...
check_disk_space <- function(min_gb = 1) {
  df_output <- system("df -g /Users/missyfrost", intern = TRUE)
  last_line <- df_output[length(df_output)]
  fields <- strsplit(trimws(last_line), "\\s+")[[1]]
  avail_gb <- as.numeric(fields[4])
  if (is.na(avail_gb) || avail_gb < min_gb) {
    stop(paste0("Low disk space: only ", avail_gb, "GB available. Aborting."))
  }
  cat("Disk space OK:", avail_gb, "GB available.\n")
}

run_batched_kaza <- function(reg_data, label) {
  n_total <- nrow(reg_data)
  batch_size <- 50000
  n_batches <- ceiling(n_total / batch_size)
  pipeline_dir <- here("data", paste0("batch_output_kaza_", label, "_", n_total))
  ...
  for (i in seq_len(n_batches)) {
    ...
    batch <- reg_data[start_row:end_row, ] |> mutate(step_id = paste(id, step_id_, sep = "_"))
  ...
  cat("\nFailed batches (", label, "):", paste(failed_batches, collapse = ", "), "\n")
...
check_disk_space(1)
ele_trk_extract_dry <- run_batched_kaza(ele_dry_reg, "dry")
saveRDS(ele_trk_extract_dry, here("outputs", "ele_trk_extract_kaza_dry.rds"))
cat("\n=== KAZA DRY COMPLETE. Rows:", nrow(ele_trk_extract_dry), "===\n")
unlink(here("data", paste0("batch_output_kaza_dry_", nrow(ele_dry_reg))), recursive = TRUE)

check_disk_space(1)
ele_trk_extract_wet <- run_batched_kaza(ele_wet_reg, "wet")
saveRDS(ele_trk_extract_wet, here("outputs", "ele_trk_extract_kaza_wet.rds"))
cat("\n=== KAZA WET COMPLETE. Rows:", nrow(ele_trk_extract_wet), "===\n")
unlink(here("data", paste0("batch_output_kaza_wet_", nrow(ele_wet_reg))), recursive = TRUE)

cat("\n=== ALL KAZA EXTRACTION COMPLETE ===\n")
